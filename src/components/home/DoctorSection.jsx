...
const bios = [
  ...
  "Dr. Elif Gizem Boyalı, Marmara Üniversitesi Periodontoloji Anabilim Dalı'ndan mezun olmuştur. Diş eti tedavileri, kemik grefti ve lazer periodontoloji alanlarında 10 yıllık uzman deneyimine sahiptir. Minimal invaziv teknikler kullanarak hastaların en konforlu tedavi sürecini yaşamasını sağlamaktadır.",
          ...
          {allDoctors.map((doc, i) => (
            ...
            >
              <div className="bg-gradient-to-br from-[#f7f3ef] to-[#e4dcd2] border border-[#d4c9bc] rounded-2xl p-5 text-center hover:border-[#8B6840]/30 transition-all flex flex-col h-full">
                ...
                <h3 className="text-sm font-bold text-[#2d2419] mb-1 leading-tight">{names[i]}</h3>
                ...
                >
                  <span>{openIndex === i ? "Biyografiyi Gizle" : "Biyografiyi Göster"}</span>
                  <ChevronDown
                    className={`w-3.5 h-3.5 transition-transform duration-300 ${openIndex === i ? "rotate-180" : ""}`}
                  />
                </button>
                  ...
                  {openIndex === i && (
                    ...
                    >
                      <div className="mt-3 pt-3 border-t border-[#d4c9bc]">
                        <p className="text-[#4a3728] text-xs leading-relaxed text-left">
                          {bios[i]}
                        </p>
                      </div>
                    </motion.div>
