...
function AutoColumn({ images, delay = 0 }) {
  ...
  return (
    <div
      className="relative w-full overflow-hidden bg-[#e8e0d5]"
  ...
  return (
    <section className="py-20 bg-[#f7f3ef] font-inter">
      <div className="max-w-6xl mx-auto px-4">
        ...
        {/* 3 Sütunlu Instagram Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          {COLUMNS.map((images, i) => (
            <div key={i} className="rounded-2xl overflow-hidden shadow-md border border-[#e4dcd2]">
              {/* Header Bar */}
              <div className="flex items-center gap-2 px-4 py-3 bg-white border-b border-[#e4dcd2]">
                <svg viewBox="0 0 24 24" className="w-5 h-5 flex-shrink-0" fill="url(#igGradCol)">
                  <defs>
                    <linearGradient id="igGradCol" x1="0%" y1="100%" x2="100%" y2="0%">
                      <stop offset="0%" stopColor="#f09433" />
                      <stop offset="25%" stopColor="#e6683c" />
                      <stop offset="50%" stopColor="#dc2743" />
                      <stop offset="75%" stopColor="#cc2366" />
                ...
                <span className="text-sm font-semibold text-[#2d2419]">PanoramaDental</span>
              </div>
              {/* Image */}
              <div className="overflow-hidden" style={{ aspectRatio: "1/1" }}>
                <AutoColumn images={images} delay={i * 600} />
              </div>
            </div>
          ))}
        </div>
