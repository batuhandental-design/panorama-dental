...
const aboutHrefs = ["/#contact", "/#services", "/#about", "/#operations"];
    ...
    <footer className="bg-[#2c2419] text-[#b0a090] font-inter">
      <div className="max-w-6xl mx-auto px-4 py-16">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          {/* Departments */}
          <div>
            <h4 className="text-[#e8dfd5] font-semibold mb-4 uppercase text-sm tracking-wider">{t.footerDepts}</h4>
            <ul className="space-y-2">
              {t.services.map((service, i) => (
                <li key={i}>
                  <a href={slugs[i]} className="text-sm hover:text-[#c9a87c] transition-colors">{service.title}</a>
                </li>
              ))}
            </ul>
          </div>
          ...
          {/* About */}
          <div>
            <h4 className="text-[#e8dfd5] font-semibold mb-4 uppercase text-sm tracking-wider">{t.footerAbout}</h4>
            <ul className="space-y-2">
              {t.footerAboutLinks.map((label, i) => (
                <li key={i}>
                  <a href={aboutHrefs[i]} className="text-sm hover:text-[#c9a87c] transition-colors">{label}</a>
                </li>
              ))}
            </ul>
          </div>
          ...
          {/* Contact */}
          <div>
            <h4 className="text-[#e8dfd5] font-semibold mb-4 uppercase text-sm tracking-wider">{t.footerContact}</h4>
            <div className="space-y-3">
              ...
              >
                ...
                <span className="text-sm text-[#25D366] group-hover:text-green-400 transition-colors font-medium">{t.footerWhatsapp}</span>
              </a>
              <div className="flex items-center gap-3">
      ...
      <div className="border-t border-white/10">
        <div className="max-w-6xl mx-auto px-4 py-6 text-center text-xs text-[#7a6e62]">
          © {new Date().getFullYear()} Haliç Panorama Dental. {t.footerCopyright}
        </div>
      </div>
    </footer>
  );
